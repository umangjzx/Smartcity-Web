...
## Backlog pass 3 — completed

11. **Full admin re-skin onto DHRUVAM tokens (High #2/#10).** Converted `admin/page.tsx`, `AdminNav.tsx`, `admin-login/page.tsx`, and `admin/layout.tsx` from the old light theme to the dark glass treatment (`bg-white/[0.04]` cards, `border-white/10`, `bg-white/5` inputs) with the real gear-wheel logo. Along the way, confirmed a genuine pre-existing bug: `text-foreground`/`text-muted-foreground` aren't registered Tailwind v4 `@theme` tokens in this project, so they compiled to no CSS — verified via `getComputedStyle` that admin headings rendered as near-white text (`rgb(247,249,252)`) on a near-white background. All ~30 occurrences replaced with explicit `text-white`/`text-white/50` classes. The four tab accents (Overview/Projects/Events/Members/Messages) move from an unrelated red/gold/blue/emerald mix to the DHRUVAM aurora palette (gold, teal, blue, emerald), and the `ui-avatars.com` fallback moved off its default pink/magenta to brand colors. Verified with `tsc`, `eslint`, `next build`, and a live login-and-click-through of all five tabs.
12. **Retrofitting `Button` onto Hero/Membership's magnetic CTAs (Medium #8).** `Button.tsx` gained a `magnetic` prop (cursor-following spring via `useMotionValue`/`useSpring`, wrapping the button in a `motion.span`) so Hero's and Membership's CTAs could move onto the shared component without losing their bespoke motion. Used `motion(Link)` rather than `motion.a` so real page navigations (not just hash anchors) keep Next's client-side routing. Verified the magnetic transform is still live on both buttons via inline `style` inspection.
